...
//				}
				//Call contract function to resend joining fee
...
//				Query getPlayer = session.createSQLQuery(
//						"select player from Tournament WHERE tournamentId NOT IN (SELECT MAX(tournamentId) from Tournament) and playerStatus='0' ORDER BY tournamentId DESC");
				Query getPlayer = session.createQuery("select player from Tournament where playerStatus='0'")
						.setFirstResult(1);
				...
				System.out.println("Final Match Fixture+++++"+dataList);
				int updateByeStatus = session.createSQLQuery("update tournament set byeStatus='1' LIMIT 1")
						.executeUpdate();
			}
			playerCount = 0;
		} catch (Exception e) {
		...
		try {
			...
			for (int i = 0; i < matchArray.size();) {
				Query insertQuery = session
						.createSQLQuery("INSERT INTO matcharray(player1address,player2address) VALUES (?,?)");
	...
	@Override
	public String updatePlaceSpell(String player, int wizardSpell1, int wizardSpell2, int wizardSpell3,
			int wizardSpell4, int wizardSpell5) {
		Session session = sessionFact.getCurrentSession();
		List<Tournament> uniqueappointment;
		int unique;
		Query findunique = session.createQuery("from Tournament where player=:player").setParameter("player", player);
...
//			List<Object> dataList1 = new ArrayList<Object>();
	...
	public String getPlayerDetails(String playerAddr) {
		...
		Query findPlayer = session.createQuery("from Tournament where player=:player").setParameter("player",
				playerAddr);
	...
	public String getTimer() {
		...
		dataList = (ArrayList<String>) session.createSQLQuery("select tournamentstarttime from FlagSettings limit 1")
				.list();
		System.out.println(dataList.get(0));
		
        Timestamp timestamp = new Timestamp(System.currentTimeMillis());
		  OffsetDateTime now = OffsetDateTime.now(ZoneOffset.UTC);
		  OffsetDateTime end = OffsetDateTime.parse(dataList.get(0));
//	        OffsetDateTime end= new OffsetDateTime(dataList.get(0));
	...
	public String calculateScore() {
		try {
			...
			for (Match match : matchFixture) {
				int wizard1RoundWin = 0;
				int wizard2RoundWin = 0;
				int wizard1RoundLoss = 0;
				int wizard2RoundLoss = 0;
				long wizard1Timestamp = 0;
				long wizard2Timestamp = 0;
				int wizard1RoundTie = 0;
				int wizard2RoundTie = 0;
				int wizard1ElementalWin = 0;
				int wizard2ElementalWin = 0;
				int wizard1ElementalLoss = 0;
				int wizard2ElementalLoss = 0;
				int wizard1WinAgainst = 0;
				int wizard2WinAgainst = 0;
				int wizard1LossAgainst = 0;
				int wizard2LossAgainst = 0;
				int wizard1NormalWin = 0;
				int wizard2NormalWin = 0;
				int wizard1NormalLoss = 0;
				int wizard2NormalLoss = 0;
				int wizard1Score = 0;
				int wizard2Score = 0;
				...
				if (wizard1RoundWin > wizard2RoundWin) {
					wizard1Status = 0;
					wizard2Status = 1;
					int updateByeStatus = session.createQuery("update Tournament set playerStatus='1' where player=:player")
							.setParameter("player", match.getPlayer2Address())
							.executeUpdate();
				} else if (wizard1RoundWin < wizard2RoundWin) {
					wizard1Status = 1;
					wizard2Status = 0;
					int updateByeStatus = session.createQuery("update Tournament set playerStatus='1' where player=:player")
							.setParameter("player", match.getPlayer1Address())
							.executeUpdate();
...
//							qry1.executeUpdate();
					...
					for (Score score : getScore) {
						if (wizard1Address.equals(score.getPlrAddress())) {
							int updateScore = session
									.createQuery("UPDATE Score SET plrStatus=:status,totalScore=:score,noOfWins=:win,"
...
//				Query queryScore = session.createSQLQuery("SELECT TOP 1 * from Score s ORDER BY s.totalScore DESC");
				...
				queryScore.setFirstResult(0);
				queryScore.setMaxResults(1);
				getScore = queryScore.list();
				for (Score score : getScore) {
					winner3 = score.getPlrAddress();
					System.out.println("Winner 3 Address"+winner3);
				}
				...
				Query getWizidQuery2 = session
						.createQuery("SELECT wizardId from Tournament where player=:playeraddress")
						.setParameter("playeraddress", wizard2Address);

				wizid2 = getWizidQuery2.uniqueResult().toString();

				Query getWizidQuery3 = session
						.createQuery("SELECT wizardId from Tournament where player=:playeraddress")
				...
				if (wizard1Status == 0) {
					...
					interaction.distributePrize(winner1, winner2, winner3);
				} else if (wizard2Status == 0) {
					winner1 = wizard2Address;
					winner2 = wizard1Address;
					interaction.distributePrize(winner1, winner2, winner3);
				}
				int changeTournamentStatus = session.createSQLQuery("update FlagSettings set tournamentStatus=:status")
						.setParameter("status", "ended").executeUpdate();
...
//		ArrayList<Object> resultScore = new ArrayList<>();
		JSONArray resultScore = new JSONArray();
		...
		for (Score score : getScore) {
			...
			JSONObject scoreTable = new JSONObject();
			Query getPlayerDetails = (Query) session.createQuery("from Tournament where player=:player")
					.setParameter("player", score.getPlrAddress());
					//.uniqueResult();
			List<Tournament> getPlayer = getPlayerDetails.list();
			int duelsPlayed = 0;
			try {
			for (Tournament tournament : getPlayer) {
				scoreTable.put("wizid",tournament.getWizardId());
				scoreTable.put("affinity",tournament.getAffinityType());
				duelsPlayed = tournament.getDuelsPlayed();
			}
			scoreTable.put("plrAddress",score.getPlrAddress());
			scoreTable.put("rank",i);
			scoreTable.put("totalScore",score.getTotalScore());
			scoreTable.put("duelsPlayed",duelsPlayed);
			scoreTable.put("noofwins",score.getNoOfWins());
			scoreTable.put("noOfLoss",score.getNoOfLoss());
			scoreTable.put("noOftie",score.getNoOftie());
			scoreTable.put("noOfWinAgainstElemental",score.getNoOfWinAgainstElemental());
			scoreTable.put("noOfLossAgainstElemental",score.getNoOfLossAgainstElemental());
			scoreTable.put("elementalWin",score.getElementalWin());
			scoreTable.put("elementalLoss",score.getElementalLoss());
			resultScore.put(scoreTable);
			i++;
			}
			catch(Exception e) {
				e.printStackTrace();
	...
	public String getContractAddress(){
		Session session = sessionFact.getCurrentSession();
		String contractAddress = (String) session.createSQLQuery("select contractAddress from FlagSettings")
				.uniqueResult();
	...
	public String getWizid(String address) {
		...
		Session session = sessionFact.getCurrentSession();
		String wizid = String.valueOf( session.createSQLQuery("select wizardid from Tournament where player=:address")
